...
class FlashCardPageState extends State<FlashCardPage> with TickerProviderStateMixin {
  ...
  List<WordDTO> allWord = Get.arguments['allWord'];
  List<WordDTO> wordList = Get.arguments['wordList'];

  Queue<WordDTO> wordQueue = Queue();
  Queue<WordDTO> laterQueue = Queue();
  Queue<WordDTO> rememberQueue = Queue();
  ...
  late Timer periodTimer = Timer(const Duration(seconds: 3), () {print("doo");});
  ...
  void videoCard(){
    print('Start timer');
    _timePanAttachCard = 1000;
    initByStatusCard();
    setState(() {
      isPlay = true;
    });
    periodTimer = Timer.periodic(const Duration(seconds: 3), (timer) {
      setState(() {
        print(isCard1);
        print(isFlipped);
        if(isCard1){
                            ...
                            AnimatedOpacity(
                                ...
                                child: Transform(
                                  ...
                                  child: GestureDetector(
                                      ...
                                      CardFront(wordQueue.first.wordName!, wordQueue.first.audio!, cardWidth, cardHeight)
                                          :
                                      CardBack(wordQueue.first.wordMean!, wordQueue.first.wordType!, wordQueue.first.image!, cardWidth, cardHeight)
                                  ),
                                )
                            ),
                          ],
                            ...
                            AnimatedOpacity(
                                ...
                                child: Transform(
                                  ...
                                  child: GestureDetector(
                                      ...
                                      child: _animationController2.value <= 0.5 ?
                                      CardFront(wordQueue.first.wordName!, wordQueue.first.audio!, cardWidth, cardHeight)
                                          :
                                      CardBack(wordQueue.first.wordMean!, wordQueue.first.wordType!, wordQueue.first.image!, cardWidth, cardHeight)
                                  ),
