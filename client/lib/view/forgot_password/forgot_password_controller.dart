...
class ForgotPasswordController extends GetxController {
  ...
  Future<void> sendRequestReset() async {
    try {
      http.Response postApi = await NetworkApiService().getApi(
          "/user/check-username?username=${emailController.text.trim()}");
  ...
  Future<void> checkOtp() async {
    try {
      print('$username');
      http.Response postApi = await NetworkApiService()
          .getApi("/user/$username/check-otp?otp=${otpController.text.trim()}");
      if (postApi.statusCode == 200) {
  ...
  Future<void> resetPassword() async {
    try {
      ...
      http.Response postApi =
          await NetworkApiService().postApi("/user/reset-password", object);
      if (postApi.statusCode == 200) {
        Get.defaultDialog(
            radius: 8,
